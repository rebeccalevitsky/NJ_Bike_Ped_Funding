...
- if obligation funds (the dollar amount that was paid back to the grantee by the USDOT or NJDOT) *are* reported and publicly available, they are usually done so in format no easily usable, such as a pdf.
...
So, for this project, we will focus on what we know.  The Federal Highway Administration (FHWA) publishes [Annual Reports](https://www.fhwa.dot.gov/environment/transportation_alternatives/annual_reports/) on TASA grant awards and obligation rates by state - though the site urges caution in using these reports to compare states, for all of the reasons listed above.  The excel sheets that house the awards are meant for excel, and excel only, and divide TASA dollars by awards for different set-asides within the larger program.  The Safe Routes to School Program and the Recreational Trails Program, for example, are both set-asides of TASA, and their awards are listed separately.   .  But, they can be cross checked with the TASA awards that NJDOT publishes on its [Local Aid Resource Center](https://njdotlocalaidrc.com/federally-funded-programs/transportation-alternatives) (a remarkably user-friendly website, as someone who has spent far too much time on other DOT websites.) So, after dedicating some time to understanding the FHWA excel reports, I compiled the awards for New Jersey into an exportable csv format, and went to work. $106,679,000 million
